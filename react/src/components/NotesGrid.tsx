...
const NotesGrid = ({
  ...
  const handleDelete = async (note: Note) => {
    ...
    try {
      let response;
      const token = isAdmin ? localStorage.getItem('adminApiKey') || '' : await getToken();
  ...
  const canEditNote = (note: Note) => {
    ...
    const noteIsPublic = typeof note.isPublic === 'string' ? note.isPublic === 'true' : note.isPublic;
    ...
    try {
      ...
      /* localStorage parse error — fall through */
    }
          ...
          return (
            <div
              ...
              style={{
                ...
                ...(!noteIsPublic && !isAdmin ? {borderLeft: '4px solid #a855f7'} : {}),
              }}
              onMouseOver={(e) => {
              ...
              {canEdit && (
                ...
                >
                  ...
                  <button
                    ...
                    onMouseOver={(e) => {
                      if (deletingId !== note.id) e.currentTarget.style.color = '#b91c1c';
                    }}
                    onMouseOut={(e) => {
      ...
      {editingNote && (
        <EditNoteModal
          note={{
            ...editingNote,
            isPublic:
              typeof editingNote.isPublic === 'string' ? editingNote.isPublic : editingNote.isPublic ? 'true' : 'false',
          }}
          isOpen={!!editingNote}
