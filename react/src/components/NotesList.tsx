...
export default function NotesList() {
  ...
  const {data: notes, loading: notesLoading, error: notesError, execute: fetchNotes} = useGetAllNotes();
  const {loading: createLoading, error: createError, execute: createNote} = useCreateNote();
  ...
  useEffect(() => {
    ...
    checkAuthAndFetch();
  }, [isSignedIn, userId, getToken, initialFetchDone, fetchNotes]);
  ...
  if (!isSignedIn) {
    return (
      <Container size='md' py='xl'>
        <Alert icon={<IconAlertCircle size='1rem' />} title='Authentication Required' color='blue'>
  ...
  if (notesError) {
    return (
      <Container size='md' py='xl'>
        <Alert icon={<IconAlertCircle size='1rem' />} title='Error Loading Notes' color='red' mb='md'>
          {String(notesError)}
        </Alert>
        <Paper withBorder p='md' radius='md'>
          ...
          <Text>Token: {authState.token ? `${authState.token.substring(0, 10)}...` : 'None'}</Text>
          <Text>Is Signed In: {isSignedIn ? 'Yes' : 'No'}</Text>
          <Button leftSection={<IconRefresh size='1rem' />} onClick={handleRefresh} mt='md' variant='light'>
            Retry
          </Button>
        </Paper>
      </Container>
  ...
  return (
    <Container size='xl' py='md'>
        ...
        <form onSubmit={handleSubmit}>
          <Grid>
            ...
            <Grid.Col span={{base: 12, sm: 4}}>
              <Box mt={25}>
                <Switch label='Make note public' checked={isPublic} onChange={(e) => setIsPublic(e.target.checked)} />
              </Box>
            </Grid.Col>
          </Grid>
          ...
          <Group justify='flex-end'>
            <Button type='submit' loading={createLoading} disabled={!noteTitle || !noteContent}>
      ...
      <Paper withBorder p='md' radius='md' mt='xl' bg='gray.0'>
        ...
        <Text size='sm'>Token: {authState.token ? `${authState.token.substring(0, 10)}...` : 'None'}</Text>
        <Text size='sm'>Is Signed In: {isSignedIn ? 'Yes' : 'No'}</Text>
        <Text size='sm'>Initial Fetch Done: {initialFetchDone ? 'Yes' : 'No'}</Text>
      </Paper>
    </Container>
  );
}
