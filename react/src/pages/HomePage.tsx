...
const HomePage = () => {
  ...
  const fetchPrivateNotes = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        const trulyPrivateNotes = data.filter((note) => note.isPublic !== 'true');
  ...
  const fetchAllNotes = useCallback(
    ...
    [adminApiKey, isAdminLoggedIn, handleAdminLogout],
  );
  ...
  }, [initialized, isSignedIn, isAdminLoggedIn, fetchPublicNotes, fetchPrivateNotes, fetchAllNotes]);
  ...
  }, [refreshTrigger, initialized, isSignedIn, isAdminLoggedIn, fetchPublicNotes, fetchPrivateNotes, fetchAllNotes]);
  ...
  }, [isAdminLoggedIn, adminApiKey, fetchAllNotes]);
    ...
    <Grid>
        ...
        <Paper shadow='sm' style={{padding: '1.5rem', borderColor: '#e5e7eb'}} withBorder radius='md'>
          ...
          {searchQuery && (
            <Text size='sm' c='dimmed' mb='xs'>
              Showing notes matching &quot;{searchQuery}&quot;
            </Text>
          )}
          ...
          <NotesGrid
            ...
            emptyMessage={
              searchQuery
                ? `No public notes match "${searchQuery}"`
                : 'No public notes yet. Be the first to create one!'
            }
            showUser={true}
      ...
      <SignedOut>
        <Grid.Col span={12}>
          <Paper
            shadow='sm'
            style={{padding: '1.5rem', borderColor: '#e5e7eb', textAlign: 'center'}}
            withBorder
            radius='md'
          >
            ...
            <SignInButton mode='modal'>
              <Button
                variant='filled'
                style={{background: '#0d9488', padding: '0.5rem 1.5rem', borderRadius: '0.5rem'}}
              >
      ...
      >
        ...
        <form onSubmit={anonymousForm.onSubmit(handleAnonymousNoteSubmit)}>
          <Stack>
            <TextInput placeholder='Note title' {...anonymousForm.getInputProps('title')} required />
            <Textarea
