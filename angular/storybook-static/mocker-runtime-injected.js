            ...
            if (isFunction) {
              let mockFunction = function () {
                if (this instanceof newContainer[property])
                  for (let { key, descriptor: descriptor2 } of getAllMockableProperties(
                    this,
                    !1,
                    options.globalConstructors,
                  )) {
...
function collectOwnProperties(obj, collector) {
  let collect = 'function' == typeof collector ? collector : (key) => collector.add(key);
...
function requireReactIs$1() {
  return (
    hasRequiredReactIs$1 ||
      ((hasRequiredReactIs$1 = 1), (reactIs$1.exports = requireReactIs_production())),
    reactIs$1.exports
  );
}
...
function requireReactIs() {
  return (
    hasRequiredReactIs ||
      ((hasRequiredReactIs = 1), (reactIs.exports = requireReactIs_production_min())),
    reactIs.exports
  );
}
    ...
    : ((hasRequiredJsTokens = 1),
      ...
      (jsTokens_1 = function* (input, { jsx = !1 } = {}) {
        ...
        for (
          ...
          ((WhiteSpace.lastIndex = lastIndex), (match = WhiteSpace.exec(input)))
...
function normalizeWindowsPath2(input = '') {
  return (
    input && input.replace(/\\/g, '/').replace(_DRIVE_LETTER_START_RE2, (r) => r.toUpperCase())
  );
}
...
function parseSingleV8Stack(raw) {
  ...
  let [url, lineNumber, columnNumber] = extractLocation(location2 ? location2[1] : sanitizedLine),
...
var { now } = Date,
  ModuleMocker = class {
    ...
    async importMock(rawId, importer) {
      await this.prepare();
      let { resolvedId, resolvedUrl, redirectUrl } = await this.rpc.resolveMock(rawId, importer, {
          mock: 'auto',
        }),
...
function registerModuleMocker(interceptor) {
  let mocker = new BuildModuleMocker(
    interceptor('__vitest_mocker__'),
    {
      ...
      resolveMock: (id, importer, options) => rpc2('resolveMock', { id, importer, options }),
