...
export const htmxController = new Elysia({prefix: '/htmx'})
  ...
  .get('/', async (ctx) => {
    try {
      ...
      // Format the response
      const formattedNotes: Note[] = publicNotesWithUsers.map(
        (item: {
      ...
      return new Response(notesPage(formattedNotes, CLERK_PUBLISHABLE_KEY, CLERK_FRONTEND_API), {
  ...
  .get('/notes', async () => {
    return new Response(notesTablePage(CLERK_PUBLISHABLE_KEY, CLERK_FRONTEND_API), {
      headers: {'Content-Type': 'text/html'},
    });
  })
  ...
  .get('/notes/:id/edit', async (ctx) => {
    try {
      ...
      // Fetch the note
      const noteResult = await typedCtx.db.select().from(notes).where(eq(notes.id, noteId)).limit(1);
  ...
  .put(
    ...
    async (ctx) => {
      try {
        ...
        const result = await typedCtx.db.update(notes).set(updatedNoteData).where(eq(notes.id, noteId)).returning();
  ...
  .get('/notes/refresh', async (ctx) => {
    try {
      ...
      return new Response(formattedNotes.map((note) => noteCard(note)).join(''), {
        headers: {'Content-Type': 'text/html'},
      });
    } catch (error) {
  ...
  .get('/admin/notes', async (ctx) => {
    try {
      ...
      if (formattedNotes.length === 0) {
        return new Response('<div class="text-center py-8 text-gray-500">No notes in the system.</div>', {
          headers: {'Content-Type': 'text/html'},
        });
      }
