...
class TestModel extends BaseApiModel<TestResource> {
  constructor() {
    // In tests we don't hit the real DB-backed BaseApiModel methods, so it's
    // safe to relax types here.
    super(testResources as any, 'id' as any);
  }
  ...
  async delete(db: any, id: string): Promise<{success: boolean; message?: string}> {
  ...
  async update(db: any, id: string, data: Partial<TestResource>): Promise<TestResource | null> {
    if (id === '1' || id === '2') {
  ...
  async findWhere(db: any, conditions: Partial<TestResource>): Promise<TestResource[]> {
...
class TestController extends BaseApiController<TestResource> {
  ...
  init(): Elysia {
    return new Elysia().group('/api', (app) =>
      app.group('/test-resources', (group) => {
        const typedApp = this.registerCommonRoutes(group as any);

        // Add create and update endpoints with a context-compatible signature
        return typedApp
          .post('', async (ctx: any) => {
            const {body, db} = ctx as {
          ...
          .get('/search', async (ctx: any) => {
            const {query, db} = ctx as {
              query: Partial<TestResource>;
              db: any;
            };
  ...
  protected async isAuthorized(userId: string | null, resourceId: string | number): Promise<boolean> {
