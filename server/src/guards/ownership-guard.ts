...
async function findOrCreateUser(clerkId: string, clerk: any, db: any) {
  try {
    ...
    const newUser = {
      clerkId: clerkId,
      email: userInfo.emailAddresses?.[0]?.emailAddress || 'unknown@example.com',
    ...
    const inserted = await db.transaction(async (tx: any) => {
      ...
      const existingUser = await tx.select().from(users).where(eq(users.clerkId, clerkId)).limit(1)
...
export const ownershipGuard = async (ctx: any) => {
  try {
    ...
    if (!auth?.userId) {
      return typedCtx.status(401, 'Unauthorized - Authentication required for ownership check')
    }
    ...
    if (!typedCtx.db) {
      return typedCtx.status(500, 'Server Error - Database connection not available')
    }

    // Safely convert resource ID to number
    const resourceIdNum = parseInt(resourceId, 10)
    if (Number.isNaN(resourceIdNum)) {
      return typedCtx.status(400, `Bad Request - Invalid resource ID format: ${resourceId}`)
    }
    ...
    if (!results || results.length === 0) {
      return typedCtx.status(403, "Forbidden - You don't have permission to access this resource")
    }

    // Store the resource for the handler to use.
    // Elysia 2 only puts `store` on context after `.state()`; initialize if missing.
    typedCtx.store ??= { resource: undefined }
