...
import type { TestDBUtils } from './db-utils'
 ...
 */
export async function createTestApp(options: CreateAppOptions): Promise<TestAppResult> {
  const {
    withAuth = true,
    withApiKey = false,
  ...
  const baseApp = new Elysia()
    .state({ resource: null as unknown })
    ...
    const mockApp = new Elysia()
      ...
      .derive(() => ({
        ...
        request: {
          headers: withApiKey
            ? {
                get: (name: string) => (name.toLowerCase() === 'x-api-key' ? apiKey : null),
              }
            : { get: () => null },
        },
      }))
