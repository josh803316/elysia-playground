 ...
 * 1. Having the backend expose /vanilla-js/env.js which sets window.__HTML5_ENV__
...
const VANILLA_ENV = window.__HTML5_ENV__ || {};
...
function loadClerkScript() {
  if (!CLERK_PUBLISHABLE_KEY) {
    console.error('[auth] Missing Clerk publishable key. Ensure CLERK_PUBLISHABLE_KEY is set on the server.');
    return Promise.reject(new Error('Missing Clerk publishable key'));
  }

  if (!CLERK_FRONTEND_API) {
    console.error('[auth] Missing Clerk frontend API host. Ensure CLERK_FRONTEND_API is set on the server.');
...
export async function initAuth() {
  ...
  console.log('[auth] window loaded. window.Clerk exists:', !!window.Clerk, 'type:', typeof window.Clerk);
